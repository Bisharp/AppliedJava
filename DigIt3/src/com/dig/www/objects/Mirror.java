	...
	public boolean interact() {
		...
		if (b == 1) {
			...
			for (Block bl : owner.getWorld()) {
				if (!bl.traversable()
						&& (owner.getCharacter().getCollisionBounds()
								.intersects(bl.getBounds()))) {
					owner.scroll(moveX ? ((tx - x)) : 0, moveY ? ((ty - y)) : 0);
					owner.getCharacter().setX(tx);
					owner.getCharacter().setY(ty);
					x = willX;
...
//				}
				...
				for (int i = 0; i < objects.size(); i++) {
					if (!(objects.get(i)==this)&&(objects.get(i).isWall()||objects.get(i) instanceof Mirror)&&objects.get(i).getBounds().intersects(lightRect)) {
